...
class PersonAux(models.Model):
    ...
    code = fields.Char(string='Person (Aux) Code', required=False)
    ...
    def _get_default_image_path(self, vals):
        ...
        image_path = get_module_resource(
            'clv_person_aux', 'static/src/img', 'person_aux-avatar.png',
        )
        return image_path
    ...
    spouse_aux_id = fields.Many2one(comodel_name='clv.person_aux', string='Spouse (Aux)', ondelete='restrict')
    father_aux_id = fields.Many2one(comodel_name='clv.person_aux', string='Father (Aux)', ondelete='restrict')
    mother_aux_id = fields.Many2one(comodel_name='clv.person_aux', string='Mother (Aux)', ondelete='restrict')
    responsible_aux_id = fields.Many2one(
        comodel_name='clv.person_aux',
        string='Responsible (Aux)',
        ondelete='restrict'
    )
    caregiver_aux_id = fields.Many2one(comodel_name='clv.person_aux', string='Caregiver (Aux)', ondelete='restrict')
