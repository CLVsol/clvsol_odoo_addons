...
"""Define the type of relations that can exist between persons."""
...
class PersonRelationType(models.Model):
    """Model that defines relation types that might exist between persons"""

    _name = "clv.person.relation.type"
    _description = "Person Relation Type"
    _order = "name"
    ...
    person_category_right = fields.Many2one(
        comodel_name="clv.person.category", string="Right person category"
    )
    allow_self = fields.Boolean(
        string="Reflexive",
        help="This relation can be set up with the same person left and " "right",
        default=False,
    )
    ...
    handle_invalid_onchange = fields.Selection(
        ...
        required=True,
        default="restrict",
        help="When adding relations criteria like person type and category"
        " are checked.\n"
    ...
    def check_existing(self, vals):
        ...
        def get_category_condition(vals, side):
            ...
            fieldname2 = "%s_person_id.category_id" % side
                ...
                if handling == "restrict":
                    raise ValidationError(
                        _(
                            ...
                            " conditions for person type or category."
                        )
                    )
    ...
    def _get_reflexive_relations(self):
        ...
        :return: a recordset of clv.person.relation.
        """
        self.env.cr.execute(
            """
            SELECT id FROM res_person_relation
            WHERE left_person_id = right_person_id
            AND type_id = %(relation_type_id)s
            """,
            {"relation_type_id": self.id},
        )
        reflexive_relation_ids = [r[0] for r in self.env.cr.fetchall()]
        return self.env["clv.person.relation"].browse(reflexive_relation_ids)
            ...
            if relations:
                raise ValidationError(
                    _(
                        ...
                        "relations defined for the following persons: "
                        "{persons}"
                    ).format(
                        relation_type=relation_type.display_name,
                        persons=relations.mapped("left_person_id.display_name"),
                    )
                )
    ...
    def create(self, vals):
        ...
        return super(PersonRelationType, self).create(vals)
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            super(PersonRelationType, rec).write(rec_vals)
    ...
    def unlink(self):
        ...
        for rec in self:
            if rec.handle_invalid_onchange == "delete":
                ...
                relations.unlink()
        return super(PersonRelationType, self).unlink()
