...
class AddressOffMassEdit(models.TransientModel):
    _description = 'Address (Off) Mass Edit'
    _name = 'clv.address_off.mass_edit'

    address_off_ids = fields.Many2many(
        comodel_name='clv.address_off',
        relation='clv_address_off_mass_edit_rel',
        string='Addresses (Off)'
    )

    global_tag_ids = fields.Many2many(
        comodel_name='clv.global_tag',
        relation='clv_address_off_mass_edit_global_tag_rel',
        column1='address_off_id',
    ...
    category_ids = fields.Many2many(
        ...
        relation='clv_address_off_mass_edit_category_rel',
        column1='address_off_id',
        column2='category_id',
        string='Categories'
    )
    ...
    marker_ids = fields.Many2many(
        ...
        relation='clv_address_off_mass_edit_marker_rel',
        column1='address_off_id',
        column2='marker_id',
        string='Markers'
    )
    ...
    def default_get(self, field_names):
        ...
        defaults['address_off_ids'] = self.env.context['active_ids']

        return defaults

    @api.multi
    def do_address_off_mass_edit(self):
        self.ensure_one()

        for address_off in self.address_off_ids:

            _logger.info(u'%s %s', '>>>>>', address_off.name)
            ...
            if self.global_tag_ids_selection == 'add':
                ...
                address_off.global_tag_ids = m2m_list
            ...
            if self.global_tag_ids_selection == 'set':
                m2m_list = []
                for global_tag_id in address_off.global_tag_ids:
                    m2m_list.append((3, global_tag_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                address_off.global_tag_ids = m2m_list
                m2m_list = []
            ...
            if self.category_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                address_off.category_ids = m2m_list
            ...
            if self.category_ids_selection == 'set':
                m2m_list = []
                for category_id in address_off.category_ids:
                    m2m_list.append((3, category_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                address_off.category_ids = m2m_list
                m2m_list = []
            ...
            if self.marker_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                address_off.marker_ids = m2m_list
            ...
            if self.marker_ids_selection == 'set':
                m2m_list = []
                for marker_id in address_off.marker_ids:
                    m2m_list.append((3, marker_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                address_off.marker_ids = m2m_list
                m2m_list = []
