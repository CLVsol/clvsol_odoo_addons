...
class Tag(models.Model):
    ...
    animal_address_history_ids = fields.Many2many(
        comodel_name='clv.animal.address.history',
        relation='clv_animal_address_history_global_tag_rel',
        column1='global_tag_id',
        column2='animal_address_history_id',
        string='Animal Address History'
    )


class AnimalAddressHistory(models.Model):
    _inherit = 'clv.animal.address.history'

    global_tag_ids = fields.Many2many(
        comodel_name='clv.global_tag',
        relation='clv_animal_address_history_global_tag_rel',
        column1='animal_address_history_id',
    ...
    def _compute_global_tag_names_suport(self):
        for r in self:
            ...
            if r.global_tag_names != global_tag_names:
                record = self.env['clv.animal.address.history'].search([('id', '=', r.id)])
