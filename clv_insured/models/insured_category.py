...
class InsuredCategory(models.Model):
    _description = 'Insured Category'
    _name = 'clv.insured.category'
    _inherit = 'clv.object.category', 'clv.code.model'

    code = fields.Char(string='Category Code', required=False)
    code_sequence = fields.Char(default='clv.insured_cat.code')
    ...
    insured_ids = fields.Many2many(
        ...
        relation='clv_insured_category_rel',
        column1='category_id',
        column2='insured_id',
        string='Insureds'
    )
    ...
    def _compute_category_names_suport(self):
        for r in self:
            ...
            if r.category_names != category_names:
                record = self.env['clv.insured'].search([('id', '=', r.id)])
