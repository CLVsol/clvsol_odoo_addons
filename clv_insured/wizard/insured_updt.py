...
class InsuredUpdate(models.TransientModel):
    ...
    global_tag_ids = fields.Many2many(
        ...
        relation='clv_insured_updt_global_tag_rel',
        column1='insured_id',
    ...
    category_ids = fields.Many2many(
        comodel_name='clv.insured.category',
        relation='clv_insured_updt_category_rel',
        column1='insured_id',
        column2='category_id',
        string='Categories'
    )
    ...
    def do_insured_updt(self):
        ...
        for insured in self.insured_ids:
            ...
            if self.global_tag_ids_selection == 'add':
                ...
                insured.global_tag_ids = m2m_list
            ...
            if self.global_tag_ids_selection == 'set':
                m2m_list = []
                for global_tag_id in insured.global_tag_ids:
                    m2m_list.append((3, global_tag_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                insured.global_tag_ids = m2m_list
                m2m_list = []
            ...
            if self.category_ids_selection == 'set':
                m2m_list = []
                for category_id in insured.category_ids:
                    m2m_list.append((3, category_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                insured.category_ids = m2m_list
                m2m_list = []
