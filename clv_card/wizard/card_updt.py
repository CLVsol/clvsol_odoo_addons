...
class CardUpdate(models.TransientModel):
    ...
    def _default_card_ids(self):
        return self._context.get('active_ids')
    card_ids = fields.Many2many(
        comodel_name='clv.card',
        relation='clv_card_updt_rel',
        string='Cards',
        default=_default_card_ids
    )

    global_tag_ids = fields.Many2many(
        comodel_name='clv.global_tag',
        relation='clv_card_updt_global_tag_rel',
        column1='card_id',
    ...
    category_ids = fields.Many2many(
        ...
        relation='clv_card_updt_category_rel',
        column1='card_id',
        column2='category_id',
        string='Categories'
    )
    ...
    def do_card_updt(self):
        ...
        for card in self.card_ids:
            ...
            if self.global_tag_ids_selection == 'set':
                m2m_list = []
                for global_tag_id in card.global_tag_ids:
                    m2m_list.append((3, global_tag_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                card.global_tag_ids = m2m_list
                m2m_list = []
