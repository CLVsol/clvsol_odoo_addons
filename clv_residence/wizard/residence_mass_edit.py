...
class ResidenceMassEdit(models.TransientModel):
    _description = 'Residence Mass Edit'
    _name = 'clv.residence.mass_edit'

    def _default_residence_ids(self):
        return self._context.get('active_ids')
    residence_ids = fields.Many2many(
        comodel_name='clv.residence',
        relation='clv_residence_mass_edit_rel',
        string='Residences',
    ...
    global_tag_ids = fields.Many2many(
        ...
        relation='clv_residence_mass_edit_global_tag_rel',
    ...
    category_ids = fields.Many2many(
        ...
        relation='clv_residence_mass_edit_category_rel',
        column1='residence_id',
        column2='category_id',
        string='Categories'
    )
    ...
    marker_ids = fields.Many2many(
        ...
        relation='clv_residence_mass_edit_marker_rel',
        column1='residence_id',
        column2='marker_id',
        string='Markers'
    )
    ...
    tag_ids = fields.Many2many(
        ...
        relation='clv_residence_mass_edit_tag_rel',
        column1='residence_id',
        column2='tag_id',
    ...
    tag_ids_selection = fields.Selection(
        [('add', 'Add'),
         ('remove_m2m', 'Remove'),
         ('set', 'Set'),
         ], string='Residence Tags:', default=False, readonly=False, required=False
    )
    ...
    def do_residence_mass_edit(self):
        ...
        for residence in self.residence_ids:
            ...
            if self.reg_state_selection == 'set':
                residence.reg_state = self.reg_state
            if self.reg_state_selection == 'remove':
                residence.reg_state = False

            if self.state_selection == 'set':
                residence.state = self.state
            if self.state_selection == 'remove':
                residence.state = False
            ...
            if self.global_tag_ids_selection == 'add':
                ...
                residence.global_tag_ids = m2m_list
            ...
            if self.global_tag_ids_selection == 'set':
                m2m_list = []
                for global_tag_id in residence.global_tag_ids:
                    m2m_list.append((3, global_tag_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                residence.global_tag_ids = m2m_list
                m2m_list = []
            ...
            if self.category_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                residence.category_ids = m2m_list
            ...
            if self.employee_id_selection == 'set':
                residence.employee_id = self.employee_id
            if self.employee_id_selection == 'remove':
                residence.employee_id = False

            if self.get_employee_id:
                if residence.ref_address_id.employee_id.id is not False:
                    residence.employee_id = residence.ref_address_id.employee_id.id

            if self.phase_id_selection == 'set':
                residence.phase_id = self.phase_id
            if self.phase_id_selection == 'remove':
                residence.phase_id = False
            ...
            if self.partner_entity_code_selection == 'set':
                if residence.entity_code != residence.code:
                    vals = {}
                    vals['entity_code'] = residence.code
                    residence.write(vals)
            if self.partner_entity_code_selection == 'remove':
                if residence.entity_code is not False:
                    vals = {}
                    vals['entity_code'] = False
                    residence.write(vals)

            if self.automatic_set_name_selection == 'set':
                residence.automatic_set_name = self.automatic_set_name
