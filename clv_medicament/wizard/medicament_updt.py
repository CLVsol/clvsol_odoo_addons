...
class MedicamentUpdate(models.TransientModel):
    _name = 'clv.medicament.updt'

    def _default_medicament_ids(self):
        return self._context.get('active_ids')
    medicament_ids = fields.Many2many(
        comodel_name='clv.medicament',
        relation='clv_medicament_updt_rel',
        string='Medicaments',
        default=_default_medicament_ids
    )

    global_tag_ids = fields.Many2many(
        comodel_name='clv.global_tag',
        relation='clv_medicament_updt_global_tag_rel',
    ...
    category_ids = fields.Many2many(
        ...
        relation='clv_medicament_updt_category_rel',
        column1='medicament_id',
        column2='category_id',
        string='Categories'
    )
    ...
    @api.multi
    def do_medicament_updt(self):
        self.ensure_one()

        for medicament in self.medicament_ids:

            _logger.info(u'%s %s', '>>>>>', medicament.name)
            ...
            if self.global_tag_ids_selection == 'add':
                ...
                medicament.global_tag_ids = m2m_list
            ...
            if self.global_tag_ids_selection == 'set':
                m2m_list = []
                for global_tag_id in medicament.global_tag_ids:
                    m2m_list.append((3, global_tag_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                medicament.global_tag_ids = m2m_list
                m2m_list = []
            ...
            if self.category_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                medicament.category_ids = m2m_list
            ...
            if self.category_ids_selection == 'set':
                m2m_list = []
                for category_id in medicament.category_ids:
                    m2m_list.append((3, category_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                medicament.category_ids = m2m_list
                m2m_list = []
