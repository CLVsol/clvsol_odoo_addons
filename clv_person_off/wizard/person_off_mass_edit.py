...
class PersonoFFMassEdit(models.TransientModel):
    _description = 'Person (Off) Mass Edit'
    _name = 'clv.person_off.mass_edit'

    person_off_ids = fields.Many2many(
        comodel_name='clv.person_off',
        relation='clv_person_off_mass_edit_rel',
        string='Persons (Off)'
    )

    global_tag_ids = fields.Many2many(
        comodel_name='clv.global_tag',
        relation='clv_person_off_mass_edit_global_tag_rel',
    ...
    category_ids = fields.Many2many(
        ...
        relation='clv_person_off_mass_edit_category_rel',
        column1='person_off_id',
        column2='category_id',
        string='Categories'
    )
    ...
    marker_ids = fields.Many2many(
        ...
        relation='clv_person_off_mass_edit_marker_rel',
        column1='person_off_id',
        column2='marker_id',
        string='Markers'
    )
    ...
    def default_get(self, field_names):
        ...
        defaults['person_off_ids'] = self.env.context['active_ids']

        return defaults

    @api.multi
    def do_person_off_mass_edit(self):
        self.ensure_one()
        ...
        for person_off in self.person_off_ids:
            ...
            if self.global_tag_ids_selection == 'add':
                ...
                person_off.global_tag_ids = m2m_list
            ...
            if self.global_tag_ids_selection == 'set':
                m2m_list = []
                for global_tag_id in person_off.global_tag_ids:
                    m2m_list.append((3, global_tag_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                person_off.global_tag_ids = m2m_list
                m2m_list = []
            ...
            if self.category_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                person_off.category_ids = m2m_list
            ...
            if self.category_ids_selection == 'set':
                m2m_list = []
                for category_id in person_off.category_ids:
                    m2m_list.append((3, category_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                person_off.category_ids = m2m_list
                m2m_list = []
            ...
            if self.marker_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                person_off.marker_ids = m2m_list
            ...
            if self.marker_ids_selection == 'set':
                m2m_list = []
                for marker_id in person_off.marker_ids:
                    m2m_list.append((3, marker_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                person_off.marker_ids = m2m_list
                m2m_list = []
