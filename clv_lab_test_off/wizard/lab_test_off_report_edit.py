...
class LabTestOffReportEdit(models.TransientModel):
    _name = 'clv.lab_test.off.report.edit'
    ...
    def _get_default(self, lab_test_type_id_code, criterion_code):
        ...
        if active_id.lab_test_type_id.code == lab_test_type_id_code:
            result = active_id.criterion_ids.search([
                ('lab_test_off_report_id', '=', active_id.id),
                ('code', '=', criterion_code),
            ]).result
        else:
            result = False
    ...
    def _default_lab_test_type_id(self):
        return self.env['clv.lab_test.off.report'].browse(self._context.get('active_id')).lab_test_type_id
    ...
    def _default_lab_test_off_request_id(self):
        return self.env['clv.lab_test.off.report'].browse(self._context.get('active_id')).lab_test_off_request_id
    lab_test_off_request_id = fields.Many2one(
        comodel_name='clv.lab_test.off.request',
        string='Lab Test (Off) Request',
        readonly=True,
        default=_default_lab_test_off_request_id
    )
    ...
    def do_report_updt(self):
        ...
        report = self.env['clv.lab_test.off.report'].browse(self._context.get('active_id'))
