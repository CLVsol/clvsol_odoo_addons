...
class LabTestOffRequest(models.Model):
    _description = 'Lab Test (Off) Request'
    _name = 'clv.lab_test.off.request'
    _rec_name = 'code'
    _order = 'code'

    code = fields.Char(string="Lab Test (Off) Request Code")

    lab_test_type_ids = fields.Many2many(
        comodel_name='clv.lab_test.type',
        relation='clv_lab_test_off_request_lab_test_type_rel',
        column1='request_id',
        column2='type_id',
        string='Lab Test (Off) Types'
    )
    lab_test_type_names = fields.Char(
        string='Lab Test (Off) Type Names',
        compute='_compute_lab_test_type_names',
        store=True
    )
    lab_test_type_names_suport = fields.Char(
        string='Lab Test (Off) Type Names Suport',
