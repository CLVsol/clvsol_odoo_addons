...
class PatientRec(models.Model):
    ...
    code = fields.Char(string='Patient (Rec) Code', required=False)
    ...
    def _check_birthday(self):
        for patient_rec in self:
            if patient_rec.birthday:
                if patient_rec.birthday > fields.Date.today():
    ...
    def _get_default_image_path(self, vals):
        ...
        image_path = get_module_resource(
            'clv_patient_rec', 'static/src/img', 'patient_rec-avatar.png',
        )
        return image_path
    ...
    # spouse_rec_id = fields.Many2one(comodel_name='clv.patient_rec', string='Spouse (Rec)', ondelete='restrict')
    # father_rec_id = fields.Many2one(comodel_name='clv.patient_rec', string='Father (Rec)', ondelete='restrict')
    # mother_rec_id = fields.Many2one(comodel_name='clv.patient_rec', string='Mother (Rec)', ondelete='restrict')
    # responsible_rec_id = fields.Many2one(
    #     comodel_name='clv.patient_rec',
    #     string='Responsible (Rec)',
    #     ondelete='restrict'
    # )
    # caregiver_rec_id = fields.Many2one(comodel_name='clv.patient_rec', string='Caregiver (Rec)', ondelete='restrict')
    ...
    def do_patient_rec_clear_address_data(self):
        ...
        for patient_rec in self:
            ...
            patient_rec.write(data_values)

    # @api.model
    # def _patient_rec_compute_age_reference_cron(self):

    #     patients_rec = self.search([("birthday", "!=", False)])
    #     patients_rec._compute_age_reference()
    #     patients_rec._compute_age_range_id()

    #     patients_rec = self.search([("estimated_age", "!=", False)])
    #     patients_rec._compute_age_reference()
    #     patients_rec._compute_age_range_id()
