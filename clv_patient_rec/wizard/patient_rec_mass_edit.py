...
class PatientRecMassEdit(models.TransientModel):
    _description = 'Patient (Rec) Mass Edit'
    _name = 'clv.patient_rec.mass_edit'

    def _default_patient_rec_ids(self):
        return self._context.get('active_ids')
    patient_rec_ids = fields.Many2many(
        comodel_name='clv.patient_rec',
        relation='clv_patient_rec_mass_edit_rel',
        string='Patients (Rec)',
    ...
    global_tag_ids = fields.Many2many(
        ...
        relation='clv_patient_rec_mass_edit_global_tag_rel',
    ...
    category_ids = fields.Many2many(
        ...
        relation='clv_patient_rec_mass_edit_category_rel',
        column1='patient_rec_id',
        column2='category_id',
        string='Categories'
    )
    ...
    marker_ids = fields.Many2many(
        ...
        relation='clv_patient_rec_mass_edit_marker_rel',
        column1='patient_rec_id',
        column2='marker_id',
        string='Markers'
    )
    ...
    patient_rec_ref_age_refresh = fields.Boolean(
    ...
    def do_patient_rec_mass_edit(self):
        ...
        for patient_rec in self.patient_rec_ids:
            ...
            if self.reg_state_selection == 'set':
                patient_rec.reg_state = self.reg_state
            if self.reg_state_selection == 'remove':
                patient_rec.reg_state = False

            if self.state_selection == 'set':
                patient_rec.state = self.state
            if self.state_selection == 'remove':
                patient_rec.state = False
            ...
            if self.global_tag_ids_selection == 'add':
                ...
                patient_rec.global_tag_ids = m2m_list
            ...
            if self.category_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                patient_rec.category_ids = m2m_list
            ...
            if self.marker_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                patient_rec.marker_ids = m2m_list
            ...
            if self.marker_ids_selection == 'set':
                m2m_list = []
                for marker_id in patient_rec.marker_ids:
                    m2m_list.append((3, marker_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                patient_rec.marker_ids = m2m_list
                m2m_list = []
            ...
            if self.phase_id_selection == 'set':
                patient_rec.phase_id = self.phase_id
            if self.phase_id_selection == 'remove':
                patient_rec.phase_id = False

            if self.partner_entity_code_selection == 'set':
                if patient_rec.entity_code != patient_rec.code:
                    vals = {}
                    vals['entity_code'] = patient_rec.code
                    patient_rec.write(vals)
            if self.partner_entity_code_selection == 'remove':
                if patient_rec.entity_code is not False:
                    vals = {}
                    vals['entity_code'] = False
                    patient_rec.write(vals)

            if self.patient_rec_ref_age_refresh:
