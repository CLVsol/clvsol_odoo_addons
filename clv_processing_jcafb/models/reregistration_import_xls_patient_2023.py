...
class AbstractProcess(models.AbstractModel):
    ...
    def _do_reregistration_import_xls_patient_2023(self, schedule):

        _logger.info(u'%s %s', '>>>>>>>> schedule:', schedule.name)

        schedule.processing_log = 'Executing: "' + '_do_reregistration_import_xls_patient_2023' + '"...\n\n'
        ...
        for i in range(sheet.nrows):
            ...
            if ok == 'x':
                ...
                _logger.info(u'>>>>>>>>>> Ok: %s, Age: %s, Name: %s, Address: %s', ok, estimated_age, patient_name, street_name)
                ...
                vals = {}
                vals['name'] = patient_name
                vals['estimated_age'] = estimated_age
