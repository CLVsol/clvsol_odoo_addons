...
class FamilyOffMassEdit(models.TransientModel):
    _description = 'Family (Off) Mass Edit'
    _name = 'clv.family_off.mass_edit'

    family_off_ids = fields.Many2many(
        comodel_name='clv.family_off',
        relation='clv_family_off_mass_edit_rel',
        string='Families (Off)'
    )

    global_tag_ids = fields.Many2many(
        comodel_name='clv.global_tag',
        relation='clv_family_off_mass_edit_global_tag_rel',
    ...
    category_ids = fields.Many2many(
        ...
        relation='clv_family_off_mass_edit_category_rel',
        column1='family_off_id',
        column2='category_id',
        string='Categories'
    )
    ...
    marker_ids = fields.Many2many(
        ...
        relation='clv_family_off_mass_edit_marker_rel',
        column1='family_off_id',
        column2='marker_id',
        string='Markers'
    )
    ...
    def default_get(self, field_names):
        ...
        defaults['family_off_ids'] = self.env.context['active_ids']

        return defaults

    @api.multi
    def do_family_off_mass_edit(self):
        self.ensure_one()

        for family_off in self.family_off_ids:

            _logger.info(u'%s %s', '>>>>>', family_off.name)
            ...
            if self.global_tag_ids_selection == 'add':
                ...
                family_off.global_tag_ids = m2m_list
            ...
            if self.global_tag_ids_selection == 'set':
                m2m_list = []
                for global_tag_id in family_off.global_tag_ids:
                    m2m_list.append((3, global_tag_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                family_off.global_tag_ids = m2m_list
                m2m_list = []
            ...
            if self.category_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                family_off.category_ids = m2m_list
            ...
            if self.category_ids_selection == 'set':
                m2m_list = []
                for category_id in family_off.category_ids:
                    m2m_list.append((3, category_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                family_off.category_ids = m2m_list
                m2m_list = []
            ...
            if self.marker_ids_selection == 'add':
                ...
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                family_off.marker_ids = m2m_list
            ...
            if self.marker_ids_selection == 'set':
                m2m_list = []
                for marker_id in family_off.marker_ids:
                    m2m_list.append((3, marker_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                family_off.marker_ids = m2m_list
                m2m_list = []
