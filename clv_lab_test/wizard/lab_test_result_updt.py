...
class LabTestResultUpdate(models.TransientModel):
    _name = 'clv.lab_test.result.updt'

    def _default_lab_test_result_ids(self):
        return self._context.get('active_ids')
    lab_test_result_ids = fields.Many2many(
        comodel_name='clv.lab_test.result',
        relation='clv_lab_test_result_updt_rel',
    ...
    global_tag_ids = fields.Many2many(
        ...
        relation='clv_lab_test_result_updt_global_tag_rel',
    ...
    @api.multi
    def do_lab_test_result_updt(self):
        self.ensure_one()
        ...
        for lab_test_result in self.lab_test_result_ids:
            ...
            if self.global_tag_ids_selection == 'add':
                ...
                lab_test_result.global_tag_ids = m2m_list
            ...
            if self.global_tag_ids_selection == 'set':
                m2m_list = []
                for global_tag_id in lab_test_result.global_tag_ids:
                    m2m_list.append((3, global_tag_id.id))
                _logger.info(u'%s %s', '>>>>>>>>>>', m2m_list)
                lab_test_result.global_tag_ids = m2m_list
                m2m_list = []
